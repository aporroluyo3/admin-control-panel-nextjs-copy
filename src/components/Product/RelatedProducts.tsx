...
import { Priority, Product, RelatedProduct, Sku } from '@/types/product';
...
export default function RelatedProducts({
  ...
  const applyFilters = (searchTerm: string) => {
    ...
    const filtered = initialNotRelatedProducts.filter((p) =>
      p.sku.toLowerCase().includes(searchTerm.toLowerCase()),
    );

    setNotRelatedProducts(filtered);
  };
  ...
  return (
    <>
      ...
      <Card title='Related products'>
        <Flex vertical align='end' gap={4} className='mb-10'>
          <Text
